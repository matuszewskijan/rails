...
WARNING: Importmaps are used only for Javascript files and can not be used for CSS delivery. Check the [Sprockets section](#how-to-use-sprockets) to learn about CSS.
...
Importmaps are essentially a string substitution for what is referred to as "bare module specifiers". They allow you to standardize the names of JavaScript module imports.
...
All of the configured import maps should be attached in `<head>` tag of your application by adding  `<%= javascript_importmap_tags %>` . The `javascript_importmap_tags` renders a bunch of scripts in the `head` tag:
...
You can use the `./bin/importmap` command that's added as part of the `importmap-rails` install to pin, unpin, or update npm packages in your import map. The binstub uses `JSPM.org`
...
Preloading is extremely useful in case you know that you'd need a specific part of the code, but you don't want to execute it immediately.
...
NOTE: The JavasScript compression will also work for your JavaScript files when you are loading your assets through `importmap-rails` or `jsbundling-rails` gems.
...
### [Webpacker/Shakapacker](https://guides.rubyonrails.org/webpacker.html)
